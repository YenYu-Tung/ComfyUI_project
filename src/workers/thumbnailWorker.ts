...
//         if (newStageLayers && newStageLayers.length > 0) {
//             // 為新階段的每個圖層創建新的畫布並複製內容
//             const restoredLayers = newStageLayers.map(layer => {
//                 const newCanvas = document.createElement('canvas');
//                 newCanvas.width = layer.canvas.width;
//                 newCanvas.height = layer.canvas.height;
//                 const ctx = newCanvas.getContext('2d');
...
//                 if (currentStage === 'stage2') {
//                     stages.stage2.thumbnailUrl = imgData;
//                 } else if (currentStage === 'stage3') {
//                     stages.stage3.thumbnailUrl = imgData;
//                 }
//                 // stages.stage1.thumbnailUrl = imgData;
//             }
//         };
...
//             // Stage 2: 處理上傳的照片和畫布
//             if (stage2Photo || stages.stage2.thumbnailUrl || stages.stage2.layers?.length > 0) {
//                 // 處理上傳的照片
//                 if (stage2Photo) {
...
//                 } else {
//                     // 如果不是當前階段，使用保存的圖層
//                     const stageLayers = stages.stage2.layers || [];
...
//             // Stage 3: 處理上傳的照片和畫布
//             if (stage3Photo || stages.stage3.thumbnailUrl || stages.stage3.layers?.length > 0) {
//                 // 處理上傳的照片
//                 if (stage3Photo) {
...
//                 } else {
//                     // 如果不是當前階段，使用保存的圖層
//                     const stageLayers = stages.stage3.layers || [];
//                     for (const layer of stageLayers) {
//                         if (layer.isVisible) {
//                             ctx.globalAlpha = layer.opacity;
//                             ctx.drawImage(layer.canvas, 0, 0, tempCanvas.width, tempCanvas.height);
...
//         const newZoom = Math.min(Math.max(canvasZoom + delta, 0.2), 0.6); // 保持實際縮放範圍
...
// // 在顏色��擇器的 onChange 事件中更新對應 stage 的背景顏色
...
// <div
//             className={
//     `w-16 h-10 rounded-[14px] flex items-center justify-center gap-1 ${!isPreviewDragging ? 'transition-colors duration-200' : ''
//     } add-preview-button`
// }
...
// <button
//     className={
//     `w-[180px] h-[41px] rounded-[14px] flex justify-center items-center gap-2 
...
// <div
//                         className={
//     `absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-20 h-20 
//                             bg-white/60 text-primary rounded-full flex items-center justify-center cursor-move
...
//         <button
//                 className={
//     `w-[90px] h-[90px] bg-[#ECECF3] rounded-[14px] relative flex-shrink-0 flex justify-center items-center ${(currentStage === 'stage2' && !stage2Photo) || (currentStage === 'stage3' && !stage3Photo)
//         ? 'shadow-inner'
//         : 'group'
//     }`
// }
